    ...
    def __init__(self, topic_name='/techman_image'):
        ...
        class _ImageNode(NodeBase):
            def __init__(inner_self, outer):
                super().__init__('aruco_p3_calib_tm_camera')
...
def estimate_single_marker_pose(marker_corners, marker_size, camera_matrix, dist_coeffs):
    """
    OpenCV-version-safe pose estimation for one ArUco marker.
    Returns rvec, tvec shaped like estimatePoseSingleMarkers output for one marker.
    """
...
def three_point_calibration(robot_points, object_points):
    ...
    x_axis_obj = object_positions[2] - object_positions[0]
    x_axis_obj = x_axis_obj / np.linalg.norm(x_axis_obj)
    xy_plane_obj = object_positions[1] - object_positions[0]
    ...
    x_axis_robot = robot_positions[2] - robot_positions[0]
    x_axis_robot = x_axis_robot / np.linalg.norm(x_axis_robot)
    xy_plane_robot = robot_positions[1] - robot_positions[0]
...
USE_SAFE_Z = True
SAFE_Z = 23.0  # mm, approach height for TM
...
# P1/P2/P3 are the marker-derived equivalents of pose_p1 / pose_p2 / pose_p3 on the table plane.
...
# Per-point trim in robot/base coordinates [mm].
# Use this when the marker-based prediction is close but not perfect.
# X/Y trim always affects both contact and approach poses.
# Z trim affects contact/calibration poses. Set APPLY_Z_TRIM_TO_APPROACH=True if you also
# want the approach/safe-Z pose for that point to move up/down by the same amount.
...
logger.info('=' * 60)
logger.info('LOADED HAND-EYE CALIBRATION RESULTS (TM)')
logger.info('=' * 60)
...
logger.info(f'ROS image topic: {ROS_IMAGE_TOPIC}')
...
def build_three_point_calibration(point_targets):
    point_map = point_targets_to_dict(point_targets)
    p1 = point_map['P1']['pose_contact']
    p2 = point_map['P2']['pose_contact']
    p3 = point_map['P3']['pose_contact']
    ...
    object_calibration_points = [
        [0.0, 0.0, 0.0, p1[3], p1[4], p1[5]],
        [0.0, p1_p2_distance, 0.0, p1[3], p1[4], p1[5]],
        [p1_p3_distance, 0.0, 0.0, p1[3], p1[4], p1[5]],
    ]
    ...
    for i, corner in enumerate(CUBE_CORNERS_BOARD):
        corner_full = [corner[0], corner[1], corner[2], p1[3], p1[4], p1[5]]
        transformed = transform_point(corner_full, T_board_to_robot)
        ...
        if i < len(EXPECTED_CORNERS_ROBOT):
            ...
            err = float(np.linalg.norm(np.array(transformed[:3]) - expected))
...
# ------------------------------------------------------------------------------
def move_robot_to_pose(robot, target_pose_mmdeg):
    logger.info(
    ...
    if ROBOT_TYPE == 'TM':
        ret = robot.move(
            ...
            speed_perc=TM_SPEED_PERC,
            acceleration_duration=TM_ACCELERATION_DURATION,
            use_precise_positioning=TM_USE_PRECISE_POSITIONING,
        )
        return ret == 0

    ret = robot.move(target_position=target_pose_mmdeg, tool_no=8, speed=50)
    return ret == 0


def _get_tm_pose_mmdeg(robot):
    status, position = robot.get_position(retries=2, retry_delay=0.05)
...
def wait_for_robot_arrival(robot, target_pose_mmdeg,
    ...
    while time.time() - start_time < max_wait:
        if ROBOT_TYPE == 'TM':
            status, current_pose = _get_tm_pose_mmdeg(robot)
        else:
            status, position = robot.get_position(times_1000=True)
            current_pose = None
            if status == 0:
                raw_pose = list(position['pos'])[:-2]
                current_pose = [
                    raw_pose[0] / 1000.0,
                    raw_pose[1] / 1000.0,
                    raw_pose[2] / 1000.0,
...
class ArUcoBoardCalibrationGuide:
    ...
    def get_current_robot_pose(self):
        if ROBOT_TYPE == 'TM':
            status, pose = _get_tm_pose_mmdeg(self.robot)
            if status == 0:
                return pose
            logger.error('Failed to get TM robot position')
            return None

        status, position = self.robot.get_position(times_1000=True)
        if status == 0:
            raw_pose = list(position['pos'])[:-2]
            return [
                raw_pose[0] / 1000.0,
    ...
    def move_calibration_point_sequence(self, point_item):
        ...
        logger.info(f"Contact pose: {point_item['pose_contact_move']}")
        logger.info(f"Approach pose: {point_item['pose_approach_move']}")

        if not move_robot_to_pose(self.robot, point_item['pose_approach_move']):
            return False
        if not wait_for_robot_arrival(self.robot, point_item['pose_approach_move']):
            return False
        verify_position(self.robot, point_item['pose_approach_move'])

        if not MOVE_TO_CONTACT_POINTS:
            logger.info('MOVE_TO_CONTACT_POINTS=False -> staying on approach/safe-Z plane. No descent to board contact pose.')
            sleep(0.3)
            return True

        if not move_robot_to_pose(self.robot, point_item['pose_contact_move']):
            return False
    ...
    def move_cube_corners(self, cube_corners_robot, cube_corners_robot_approach):
        ...
        for i, (contact_pose, approach_pose) in enumerate(zip(cube_corners_robot, cube_corners_robot_approach)):
            full_pose = list(contact_pose)
            approach = list(approach_pose)
            logger.info(
                f"\nCube corner {i+1}: contact=({full_pose[0]:.3f}, {full_pose[1]:.3f}, {full_pose[2]:.3f}) mm | "
    ...
    def display_overlay(self, frame, success, rvec=None, tvec=None, marker_pose_base=None,
                        center_pose=None, point_targets=None, calibration_report=None):
        display = frame.copy()
        if success and rvec is not None and tvec is not None:
            ...
            if point_targets:
                ...
                for item in point_targets:
                    ...
                    cv2.putText(display, f"{prefix}{item['name']} C: ({pose[0]:.1f}, {pose[1]:.1f}, {pose[2]:.1f})",
                                (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.48, (255, 160, 255), 1)
                    y += 18
                    appr = item['pose_approach_move']
                    cv2.putText(display, f"  {item['name']} A: ({appr[0]:.1f}, {appr[1]:.1f}, {appr[2]:.1f})",
        ...
        footer1 = '1/2/3 select P1/P2/P3 | SPACE move selected point | c compute 3-point eval'
        footer2 = 'e run cube-edge movement | i return initial pose | q quit'
    ...
    def run(self):
        try:
            ...
            logger.info('\n' + '=' * 60)
            logger.info('MARKER-BASED 3-POINT BOARD CALIBRATION + CUBE EVALUATION (TM)')
            logger.info('=' * 60)
            ...
            while True:
                ...
                cv2.imshow('Marker 3-Point Calibration TM', display)
                ...
                if key == ord('1'):
                    ...
                    if last_success and last_point_targets:
                        last_calibration_report = build_three_point_calibration(last_point_targets)
...
def main():
    print('=' * 60)
    print('MARKER-BASED 3-POINT BOARD CALIBRATION + CUBE EVALUATION (TM)')
    print('=' * 60)
    print('\nThis script:')
    ...
    print('  4) builds approach poses using SAFE_Z for motion only,')
    ...
    print('  SPACE     : move selected point')
    print('  c         : compute 3-point calibration + cube evaluation')
