// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from techman_robot_msgs:srv/TechmanRobotCommand.idl
// generated code does not contain a copyright notice
#include "techman_robot_msgs/srv/detail/techman_robot_command__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"

// Include directives for member types
// Member `command`
// Member `command_parameter_string`
#include "rosidl_runtime_c/string_functions.h"

bool
techman_robot_msgs__srv__TechmanRobotCommand_Request__init(techman_robot_msgs__srv__TechmanRobotCommand_Request * msg)
{
  if (!msg) {
    return false;
  }
  // command
  if (!rosidl_runtime_c__String__init(&msg->command)) {
    techman_robot_msgs__srv__TechmanRobotCommand_Request__fini(msg);
    return false;
  }
  // command_parameter_string
  if (!rosidl_runtime_c__String__init(&msg->command_parameter_string)) {
    techman_robot_msgs__srv__TechmanRobotCommand_Request__fini(msg);
    return false;
  }
  return true;
}

void
techman_robot_msgs__srv__TechmanRobotCommand_Request__fini(techman_robot_msgs__srv__TechmanRobotCommand_Request * msg)
{
  if (!msg) {
    return;
  }
  // command
  rosidl_runtime_c__String__fini(&msg->command);
  // command_parameter_string
  rosidl_runtime_c__String__fini(&msg->command_parameter_string);
}

bool
techman_robot_msgs__srv__TechmanRobotCommand_Request__are_equal(const techman_robot_msgs__srv__TechmanRobotCommand_Request * lhs, const techman_robot_msgs__srv__TechmanRobotCommand_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // command
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->command), &(rhs->command)))
  {
    return false;
  }
  // command_parameter_string
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->command_parameter_string), &(rhs->command_parameter_string)))
  {
    return false;
  }
  return true;
}

bool
techman_robot_msgs__srv__TechmanRobotCommand_Request__copy(
  const techman_robot_msgs__srv__TechmanRobotCommand_Request * input,
  techman_robot_msgs__srv__TechmanRobotCommand_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // command
  if (!rosidl_runtime_c__String__copy(
      &(input->command), &(output->command)))
  {
    return false;
  }
  // command_parameter_string
  if (!rosidl_runtime_c__String__copy(
      &(input->command_parameter_string), &(output->command_parameter_string)))
  {
    return false;
  }
  return true;
}

techman_robot_msgs__srv__TechmanRobotCommand_Request *
techman_robot_msgs__srv__TechmanRobotCommand_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  techman_robot_msgs__srv__TechmanRobotCommand_Request * msg = (techman_robot_msgs__srv__TechmanRobotCommand_Request *)allocator.allocate(sizeof(techman_robot_msgs__srv__TechmanRobotCommand_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(techman_robot_msgs__srv__TechmanRobotCommand_Request));
  bool success = techman_robot_msgs__srv__TechmanRobotCommand_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
techman_robot_msgs__srv__TechmanRobotCommand_Request__destroy(techman_robot_msgs__srv__TechmanRobotCommand_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    techman_robot_msgs__srv__TechmanRobotCommand_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
techman_robot_msgs__srv__TechmanRobotCommand_Request__Sequence__init(techman_robot_msgs__srv__TechmanRobotCommand_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  techman_robot_msgs__srv__TechmanRobotCommand_Request * data = NULL;

  if (size) {
    data = (techman_robot_msgs__srv__TechmanRobotCommand_Request *)allocator.zero_allocate(size, sizeof(techman_robot_msgs__srv__TechmanRobotCommand_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = techman_robot_msgs__srv__TechmanRobotCommand_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        techman_robot_msgs__srv__TechmanRobotCommand_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
techman_robot_msgs__srv__TechmanRobotCommand_Request__Sequence__fini(techman_robot_msgs__srv__TechmanRobotCommand_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      techman_robot_msgs__srv__TechmanRobotCommand_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

techman_robot_msgs__srv__TechmanRobotCommand_Request__Sequence *
techman_robot_msgs__srv__TechmanRobotCommand_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  techman_robot_msgs__srv__TechmanRobotCommand_Request__Sequence * array = (techman_robot_msgs__srv__TechmanRobotCommand_Request__Sequence *)allocator.allocate(sizeof(techman_robot_msgs__srv__TechmanRobotCommand_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = techman_robot_msgs__srv__TechmanRobotCommand_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
techman_robot_msgs__srv__TechmanRobotCommand_Request__Sequence__destroy(techman_robot_msgs__srv__TechmanRobotCommand_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    techman_robot_msgs__srv__TechmanRobotCommand_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
techman_robot_msgs__srv__TechmanRobotCommand_Request__Sequence__are_equal(const techman_robot_msgs__srv__TechmanRobotCommand_Request__Sequence * lhs, const techman_robot_msgs__srv__TechmanRobotCommand_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!techman_robot_msgs__srv__TechmanRobotCommand_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
techman_robot_msgs__srv__TechmanRobotCommand_Request__Sequence__copy(
  const techman_robot_msgs__srv__TechmanRobotCommand_Request__Sequence * input,
  techman_robot_msgs__srv__TechmanRobotCommand_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(techman_robot_msgs__srv__TechmanRobotCommand_Request);
    techman_robot_msgs__srv__TechmanRobotCommand_Request * data =
      (techman_robot_msgs__srv__TechmanRobotCommand_Request *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!techman_robot_msgs__srv__TechmanRobotCommand_Request__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          techman_robot_msgs__srv__TechmanRobotCommand_Request__fini(&data[i]);
        }
        free(data);
        return false;
      }
    }
    output->data = data;
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!techman_robot_msgs__srv__TechmanRobotCommand_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


bool
techman_robot_msgs__srv__TechmanRobotCommand_Response__init(techman_robot_msgs__srv__TechmanRobotCommand_Response * msg)
{
  if (!msg) {
    return false;
  }
  // is_success
  return true;
}

void
techman_robot_msgs__srv__TechmanRobotCommand_Response__fini(techman_robot_msgs__srv__TechmanRobotCommand_Response * msg)
{
  if (!msg) {
    return;
  }
  // is_success
}

bool
techman_robot_msgs__srv__TechmanRobotCommand_Response__are_equal(const techman_robot_msgs__srv__TechmanRobotCommand_Response * lhs, const techman_robot_msgs__srv__TechmanRobotCommand_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // is_success
  if (lhs->is_success != rhs->is_success) {
    return false;
  }
  return true;
}

bool
techman_robot_msgs__srv__TechmanRobotCommand_Response__copy(
  const techman_robot_msgs__srv__TechmanRobotCommand_Response * input,
  techman_robot_msgs__srv__TechmanRobotCommand_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // is_success
  output->is_success = input->is_success;
  return true;
}

techman_robot_msgs__srv__TechmanRobotCommand_Response *
techman_robot_msgs__srv__TechmanRobotCommand_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  techman_robot_msgs__srv__TechmanRobotCommand_Response * msg = (techman_robot_msgs__srv__TechmanRobotCommand_Response *)allocator.allocate(sizeof(techman_robot_msgs__srv__TechmanRobotCommand_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(techman_robot_msgs__srv__TechmanRobotCommand_Response));
  bool success = techman_robot_msgs__srv__TechmanRobotCommand_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
techman_robot_msgs__srv__TechmanRobotCommand_Response__destroy(techman_robot_msgs__srv__TechmanRobotCommand_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    techman_robot_msgs__srv__TechmanRobotCommand_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
techman_robot_msgs__srv__TechmanRobotCommand_Response__Sequence__init(techman_robot_msgs__srv__TechmanRobotCommand_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  techman_robot_msgs__srv__TechmanRobotCommand_Response * data = NULL;

  if (size) {
    data = (techman_robot_msgs__srv__TechmanRobotCommand_Response *)allocator.zero_allocate(size, sizeof(techman_robot_msgs__srv__TechmanRobotCommand_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = techman_robot_msgs__srv__TechmanRobotCommand_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        techman_robot_msgs__srv__TechmanRobotCommand_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
techman_robot_msgs__srv__TechmanRobotCommand_Response__Sequence__fini(techman_robot_msgs__srv__TechmanRobotCommand_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      techman_robot_msgs__srv__TechmanRobotCommand_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

techman_robot_msgs__srv__TechmanRobotCommand_Response__Sequence *
techman_robot_msgs__srv__TechmanRobotCommand_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  techman_robot_msgs__srv__TechmanRobotCommand_Response__Sequence * array = (techman_robot_msgs__srv__TechmanRobotCommand_Response__Sequence *)allocator.allocate(sizeof(techman_robot_msgs__srv__TechmanRobotCommand_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = techman_robot_msgs__srv__TechmanRobotCommand_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
techman_robot_msgs__srv__TechmanRobotCommand_Response__Sequence__destroy(techman_robot_msgs__srv__TechmanRobotCommand_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    techman_robot_msgs__srv__TechmanRobotCommand_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
techman_robot_msgs__srv__TechmanRobotCommand_Response__Sequence__are_equal(const techman_robot_msgs__srv__TechmanRobotCommand_Response__Sequence * lhs, const techman_robot_msgs__srv__TechmanRobotCommand_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!techman_robot_msgs__srv__TechmanRobotCommand_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
techman_robot_msgs__srv__TechmanRobotCommand_Response__Sequence__copy(
  const techman_robot_msgs__srv__TechmanRobotCommand_Response__Sequence * input,
  techman_robot_msgs__srv__TechmanRobotCommand_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(techman_robot_msgs__srv__TechmanRobotCommand_Response);
    techman_robot_msgs__srv__TechmanRobotCommand_Response * data =
      (techman_robot_msgs__srv__TechmanRobotCommand_Response *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!techman_robot_msgs__srv__TechmanRobotCommand_Response__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          techman_robot_msgs__srv__TechmanRobotCommand_Response__fini(&data[i]);
        }
        free(data);
        return false;
      }
    }
    output->data = data;
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!techman_robot_msgs__srv__TechmanRobotCommand_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
